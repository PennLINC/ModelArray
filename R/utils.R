...
#'
#' @param ofInterest got via: gam.formula.breakdown
#'  <- mgcv::interpret.gam(formula); ofInterest <- gam.formula.breakdown$smooth.spec[[i]]
#' @importFrom mgcv s
#' @importFrom dplyr %>%
#' @importFrom crayon black
#' @noRd
...
#'
#' @details
#' Why a separate function is needed for t(), cannot using s(): in ofInterest, "fx" is "fx"
#' for t(), but "fixed" for s() - so they are different.
...
checker_gam_formula <- function(formula, gam.formula.breakdown, onemodel = NULL) {
  ...
  if (length(gam.formula.breakdown$smooth.spec) != 0) { # if there is smooth term
    ...
    for (i_smoothTerm in seq_along(gam.formula.breakdown$smooth.spec)) {
      ...
      # check what class of smooth term (s or ti or ?);
      # then call the corresponding function - throw out the message for important argument for this class
...
#' where \code{y} is \code{response.var}, \code{x} is \code{smooth.var}, and
#' \code{orderedFactor} is \code{factor.var} - see \code{factor.var} for more.
...
#' @param factor.var character class, the variable name for factor.
#'  It should be an ordered factor. If not, it will generate it as a new column in `phenotypes`,
#'  which requires `reference.group`.
#' @param smooth.var character class, the variable name in smooth term as main effect
#' @param phenotypes data.frame class, the cohort matrix with columns of independent variables
#'   (including \code{factor.var} and \code{smooth.var}) to be added to the model
#' @param reference.group character class, the reference group for ordered factor of `factor.var`;
#'   required when `factor.var` in `phenotypes` is not an ordered factor.
#' @param prefix.ordered.factor character class, the prefix for ordered factor;
#'   required when `factor.var` in `phenotypes` is not an ordered factor.
...
#' @return a list, including: 1) formula generated; 2) data.frame phenotypes -
...
gen_gamFormula_factorXsmooth <- function(response.var, factor.var, smooth.var, phenotypes,
  ...
  if (
    ...
    phenotypes[[factor.var]] <- ordered(
      phenotypes[[unordered.factor.var]],
      levels = list.groups
