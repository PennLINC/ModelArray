...
Execution halted
```
One reason this happen is, means that the HDF5 file you want to "load" does have some results, but you did not specify the result name, and `ModelArray()` function uses the default name `myAnalysis` which is not the result name you have.
