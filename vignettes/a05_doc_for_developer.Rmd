...
* test the package and related documentations and websites can be built, and update any documentations: run `devtools::build_readme()`, `devtools::check()`, `devtools::build()`, `devtools::build_vignettes()`, and finally `pkgdown::build_site()`. Please check whether there is no error along the process, and the final built sites look good.
...
To easily show and access an `ModelArray` object, we adopt S4 Object Oriented Programming (OOP) model. 

End users actually are using such S4 methods frequently. For example, when showing the (summarized) content of an object, users simply enter its name in R console and press "enter". It means to use the generic `show()` method. As for showing an `ModelArray` object, we adopt the same way. As `show()` has already been a generic function, we only need to `setMethod()` for its setup.
...
The general structure for model fitting functions `ModelArray.<model_name>()` in `ModelArray`: 
...
All the test files are located in folder `tests/testthat`. Once a commit is pushed to GitHub, Circle CI (a CI testing platform) will be triggered to build the software and run the unit tests.  

The tests serve the goal of:
...
Developers can test locally to see if tests in a test file (in `tests/testthat` folder) without errors:
