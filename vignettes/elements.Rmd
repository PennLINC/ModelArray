---
title: "Elements: The Unit of Analysis"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Elements: The Unit of Analysis}
...
ModelArray performs mass-univariate statistical analysis - 
fitting the same model independently at every spatial location in the brain. 
We call each spatial location an **element**. 
Depending on the imaging modality, an element is one of:
...
Despite the differences in what they represent, ModelArray treats all of these uniformly.
The same functions, `ModelArray.lm()`, `ModelArray.gam()`, `ModelArray.wrap()`, 
work regardless of whether your elements are fixels, voxels, or greyordinates.

## The scalar matrix

At the core of every ModelArray object is a **scalar matrix** stored in an HDF5 file. 
This matrix has:
...
This is a `DelayedMatrix`, meaning the data lives on disk in the HDF5 file and is only read into memory when accessed.
This is what makes ModelArray memory-efficient — see `vignette("hdf5-large-analyses")` for details.

## Element IDs

Every element has an integer **element ID** starting at 0. 
The element ID corresponds directly to the row index in the scalar matrix
(row 1 = element ID 0, row 2 = element ID 1, and so on).

When you run a model with ModelArray, 
the output data frame includes an `element_id` column that maps results back to their spatial location:

```{r element_ids, eval=FALSE}
...
The `element.subset` parameter, available in `ModelArray.lm()`, `ModelArray.gam()`,
and `ModelArray.wrap()`, lets you run on a subset of elements rather than all of them. 
This is useful for:
...
For voxel-wise or vertexwise data, different subjects may have different brain coverage (subject-specific masks).
This means some elements near the edge of the brain may not have valid data from all subjects.
...
A voxel is only analyzed if the number of subjects with valid (non-NaN) values exceeds **both** thresholds.
Otherwise, the voxel is skipped and its outputs are set to `NaN`.

```{r voxel_thresholds, eval=FALSE}
