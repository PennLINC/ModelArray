...
fitting the same model independently at every spatial location within the brain mask. 
...
The same analysis functions - `ModelArray.lm()`, `ModelArray.gam()`, and `ModelArray.wrap()` -  
...
the output data frame includes an `element_id` column to maps results the spatial location of each element:
...
and `ModelArray.wrap()`, lets you run models on a subset of elements rather than all of them. 
This is useful for:
...
For voxel-wise or vertexwise data, subjects may have different brain coverage (subject-specific masks).
This means some elements, particularly near the edge of the analysis mask, may not have valid data from all subjects.
