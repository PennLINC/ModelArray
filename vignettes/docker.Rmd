...
Here we cover how to use this Docker image to run `ModelArray` and `ConFixel`. Before going through this documentation, please download demo fixel-wise data we provide from OSF into a directory e.g., `/myProject` - please refer to Step 1.1 and Step 1.2 in `vignette("walkthrough")`. The data we provide includes subject-level fixel-wise FDC data (.mif files) and a .csv file of cohort phenotypes:
...
We recommend to first prepare an R script of running ModelArray. To prepare it, you may try out the commands of `ModelArray` either on your local computer, or in the interactive R console that is launched by `singularity run --cleanenv modelarray_confixel_latest.sif R`. 

TODO: ^^ try out if we can ModelArray something by `singularity run .... R` - any issue without mounting?
...
mylm <- ModelArray.lm(formula.lm, modelarray, phenotypes, "FDC",
                      n_cores = 4)   # Make sure you also request >=4 CPU cores on the cluster
# Above is a full run of all fixels which will take some time; if you want to quickly test out first, add `element.subset = 1:100` in `ModelArray.lm()`
...
Let's name this R script as `run_ModelArray.R` and save it to `/myProject` folder.
...
singularity run --cleanenv ${fn_singularity} \
    Rscript ./${filename_Rscript}
# Above command will take some time if running the all fixels.
```

You may run above command in an interactive terminal. To avoid the connection disruption that stops the command running, we suggest saving above commands into a bash file and submitting a job to run this on the cluster. 
