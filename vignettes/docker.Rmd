...
As you probably noted, we need to "mount" directories that are on the cluster but outside the singularity container into directories within the container. This is done by `-B` when `singularity run`.  
...
In addition, Note that for `--relative-root` we provide the directory within the singularity container. Other argument paths should be relative to `--relative-root` **within** the container, not outside, i.e., imagine you're inside the container and in the directory of `--relative-root`, where are those files relative to here?
...
Let's name this R script as `run_ModelArray.R`, and save this R script to `/myProject/code` folder, so the current folder structure is:
```{.console}
/myProject
├── code
│   ├── run_ModelArray.R
...
You may run above command in an interactive terminal, however, this is probably not the best practice. To avoid the connection disruption that stops the command from running, we suggest saving above commands into a bash file (e.g., `call_ModelArray.sh` in `code` folder) and submitting a job to run this on the cluster. Below are example commands for SGE clusters:

```
cd /myProject/code    # make sure you're in the same directory as `call_ModelArray.sh`

h_vmem=10G   
num_cores=4   # MAKE SURE IT'S CONSISTENT WITH pncNback_call_modelarrayR.sh!!!

qsub -l h_vmem=${h_vmem} -pe threaded ${num_cores} call_ModelArray.sh
```

Here, `h_vmem` is the (max) virtual memory we need, which is usually higher than the real memory the program takes, therefore we suggest giving a bit redundancy just to be safe. Increase this value if you are requesting more CPU cores (`num_cores`).
