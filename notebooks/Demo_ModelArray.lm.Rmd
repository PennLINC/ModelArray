...
lm.outputs <- ModelArray.lm (formula, fixelarray, phenotypes, scalar, grid.subset = grid.subset, 
                             full.outputs = TRUE,   # +++++++++++++++++++++
...
# max(Rprofsummary_both$by.total$mem.total)   # grid.subset=1:100: 27.0MB BUT object.size of fixelarray (15.5KB) and phenotypes (11.3KB); 1:1000 = 42.5MB BUT object.size(lm.outputs)=90KB, even object.size(fits)=184KB
...
# 
# # prof  <- lineprof(ModelArray.lm (formula, fixelarray, phenotypes, scalar, grid.subset = grid.subset, 
...
# 
# # peakRAM_lm <- peakRAM(ModelArray.lm (formula, fixelarray, phenotypes, scalar, grid.subset = grid.subset, 
# #                              full.output = full.output,  var.terms = var.terms, var.model = var.model, 
# #                              correct.p.value.terms = "none",
# #                              correct.p.value.model = c("fdr","bonferroni"),
# #                               verbose = TRUE, pbar = TRUE, n_cores = 2))
# 
# # profvis_lm <- profvis({
# #   ModelArray.lm (formula, fixelarray, phenotypes, scalar, grid.subset = grid.subset,
# #                              full.output = full.output,  var.terms = var.terms, var.model = var.model,
# #                              correct.p.value.terms = "none",
# #                              correct.p.value.model = c("fdr","bonferroni"),
# #                               verbose = TRUE, pbar = TRUE, n_cores = 2)
