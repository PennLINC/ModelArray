...
ggplot(clean.df.multi, aes(x = Est.h., y = memory_GB, fill = process)) + 
  ...
  xlab("Time (hour)") +
  ylab("Memory (GB)") +
  ggtitle(paste0("FixelArray.lm(): ncores=", toString(num.cores), ", nsubj=", toString(num.subj)))
...
```{r}
#folder <- "D:\\Research\\Satterthwaite_lab\\fixel_project\\FixelArray_benchmark\\lm.josiane.nfixel-0.nsubj-30.ncore-4.vmware.runMemProfiler.s-0.1sec.20210901-140053"  # s=0.1sec, time is long....
 folder <- "D:\\Research\\Satterthwaite_lab\\fixel_project\\FixelArray_benchmark\\lm.josiane.nfixel-0.nsubj-30.ncore-4.vmware.runMemProfiler.s-1sec.20210903-102106"
...
ggplot(clean.df.multi[clean.df.multi$Est.h.>1.45, ], aes(x = Est.h., y = memory_GB, fill = process)) + 
  ...
  xlab("Time (hour)") +
  ylab("Memory (GB)") +
  ggtitle(paste0("FixelArray.lm(): ncores=", toString(num.cores), ", nsubj=", toString(num.subj), ", close to end"))
...
if ((num.subj == nsubj) && (all(summary.when.max$num.cores == summary.max$nthreads))) {
  together.max.total.GB = data.frame(nparallel = summary.when.max$num.cores,
                                  FixelArray = summary.when.max$total.RSS.MB./1024,
                                  MRtrix = summary.max$max.RSS.GB.)
                      
  
} else {
  message("number of cores/threads are different in profiling FixelArray vs MRtrix!")
}
...
Josiane's dataset, all fixels (~600k), num.subj = 938, set up by installing FixelArray package (professional way)
```{r}
main.folder <- "D:\\Research\\Satterthwaite_lab\\fixel_project\\FixelArray_benchmark"
...
ggplot(clean.summary.when.max, aes(fill=process, y=memory_MB/1024, x=num.subj)) + 
  ...
  xlab("Number of subjects") +
  ylab("Max memory (GB)") +
  ggtitle(paste0("FixelArray.lm(): ncores = ", toString(num.cores)))
...
# saved p values as .txt files via Confixel/notebooks/compare_MRtrix_FixelArray.py
