...
Run FixelArray.lm:
```{r}
lm.outputs <- FixelArray.lm (formula, data, phenotypes, scalar, fixel.subset = fixel.subset, 
                              var.terms = var.terms, 
                              var.model = var.model, 
                              overwrite = TRUE,
...
Save to .h5 file:
```{r}
writeResults.enh(fn.output, df.output = lm.outputs, analysis_name=analysis_name, overwrite=TRUE)
