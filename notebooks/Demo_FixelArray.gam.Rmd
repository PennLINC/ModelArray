...
if (flag_whichdataset == "test_n50") {
  formula <- FD ~ s(age, k=4) + sex # ++++ to do: add motion quantification
...
onemodel.tidy.smoothTerm <- onemodel %>% broom::tidy()   #  !!! if formula is FD ~ age without s(), onemodel.tidy.smoothTerm will be empty....
onemodel.tidy.parametric <- onemodel %>% broom::tidy(parametric = TRUE)  # different column names from .smoothTerm....

onemodel.glance <- onemodel %>% broom::glance()



onemodel.tidy.smoothTerm
onemodel.tidy.parametric
onemodel.glance
onemodel.summary
