...
# Usual application
```{r}
mygam <- FixelArray.gam(FD ~ s(age) + s(factorA), data = fixelarray, phenotypes = phenotypes, scalar = scalar, fixel.subset = fixel.subset,
                        eff.size.term.index = c(1),
                        n_cores=2, pbar = TRUE)
mygam
```
...
# Test eff.size
```{r}
mygam_effSize <- FixelArray.gam(FD ~ s(age) + s(factorA), data = fixelarray, phenotypes = phenotypes, scalar = scalar, fixel.subset = fixel.subset,
                                 var.smoothTerms = c("statistic","p.value","eff.size"),
                                 eff.size.term.index = c(1,2),
                                  n_cores = 2, pbar = FALSE) 
