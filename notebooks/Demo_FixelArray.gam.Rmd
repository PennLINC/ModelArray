...
# defaults:
mygam <- FixelArray.gam(formula = formula, data = fixelarray, phenotypes = phenotypes, scalar = scalar, 
                           fixel.subset = fixel.subset, 
...
# full.outputs = TRUE:
```{r}
mygam_allOutputs <- FixelArray.gam(formula = formula, data = fixelarray, phenotypes = phenotypes, scalar = scalar, 
                           fixel.subset = fixel.subset, 
                           full.outputs = TRUE,
...
fixelarray_josiane_full <- FixelArray(fn.josiane.full, scalar_types = c("FDC"), analysis_names = "gam_allOutputs")
fixelarray_josiane_full@results$gam_allOutputs$results_matrix 
colnames(fixelarray_josiane_full@results$gam_allOutputs$results_matrix )

```

# Play around with mgcv::gam() - blocks in analyseOneFixel.gam()
```{r}
