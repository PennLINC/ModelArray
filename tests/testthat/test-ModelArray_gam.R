test_that("test that ModelArray.gam() works as expected", {
  ...
  # scalar_name = c("FD")
  # modelarray <- ModelArray(h5_path, scalar_types = scalar_name,
  # analysis_names = c("my_analysis"))
  ...
  # ordered factor, "F" as reference group
  # phenotypes$sexFactor <- factor(phenotypes$sex, levels = unique(phenotypes$sex))
  # factor but not ordered
  ...
  # extract the last several characters in a string - ref:
  # https://stackoverflow.com/questions/7963898/
  ...
  compare_expected_results <- function(actual, expected, idx.fixel = idx.fixel.gam) {
    ...
    for (name.p.adjust in p.adjust.methods) {
      ...
      if (name.p.adjust %in% substrRight(col.names, nchar(name.p.adjust))) {
        # if any column name's last several characters matches (to avoid
        # name.p.adjust = "BY" vs interation term's name) message("detected!")
        list.idx <- stringr::str_which(col.names, name.p.adjust)
        # the index of the columns that contains this name.p.adjust
        for (idx in list.idx) {
  ...
  ### functions for effect size ##### calculating partial rsq for modelarray
  # (this is not comprehensive; does not include additional methods; focus is to
  # check if partial rsq is calculated correctly)
  partialRsq_gam_modelarray <- function(i_element, modelarray, phenotypes, scalar, full.formula, reduced.formula) {
    ...
    # calculating SSE: used observed y (i.e. excluding observations with NA),
    # and fitted values, directly from model object
  ...
  expect_equal(mygam$element_id, (min(element.subset) - 1):(max(element.subset) - 1))
  ...
  expect_equal(
    as.numeric(dim(mygam_default)),
    c(
      length(element.subset),
      1 + 1 * (length(var.smoothTerms.default) + 1) +
        2 * (length(var.parametricTerms.default) + 1) +
        length(var.model.default)
    )
  )

  mygam_fullOutputs <- ModelArray.gam(
    FD ~ s(age) + sex,
    data = modelarray,
  ...
  expect_equal(
    as.numeric(dim(mygam_fullOutputs)),
    c(
      length(element.subset),
      1 + 1 * (length(var.smoothTerms.full) + 1) +
        2 * (length(var.parametricTerms.full) + 1) +
        length(var.model.full)
    )
  )

  ### when there is no term or stat output ##### if there is no explicit
  # parametric term (although there will be term "Intercept") or parametric stat:
  ...
  expect_warning(mygam_noParamStat <- ModelArray.gam(FD ~ s(age) + sex,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
    var.parametricTerms = c(),
    # there will be warning: p.value was not included in var.parametricTerms, so
    # not to perform its p.value corrections
    n_cores = 2,
    pbar = FALSE
  ))
  # if there is no smooth term or stat:
  mygam_noSmoothTerm <- ModelArray.gam(FD ~ age,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
    n_cores = 2,
    pbar = FALSE
  )
  # should without error or warning, as it's just a message # expect_output() to
  # test output "there is no smooth term in the requested formula" does not work
  ...
  expect_warning(mygam_noSmoothStat <- ModelArray.gam(FD ~ s(age) + sex,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
    var.smoothTerms = c(),
    # warning: p.value was not included in var.smoothTerms, so not to perform
    # its p.value corrections
    n_cores = 2,
    pbar = FALSE
  ))
  ...
  # if there is only one var.* not empty: (test with request of reduced model -
  # where there will calls as: c(), c(), c("adj.r.squared"))
  w <- capture_warnings(mygam_onlyModelStat <- ModelArray.gam(
    FD ~ s(age) + sex,
    data = modelarray,
    phenotypes = phenotypes,
    scalar = scalar_name,
  ...
  # all=FALSE: only one element instead of all elements in actual value need to
  # match)
  expect_match(w, "p.value was not included in var.parametricTerms", all = FALSE)
  w <- capture_warnings(mygam_onlyParametricTermsStat <- ModelArray.gam(FD ~ s(age) + sex,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
  ...
  # match)
  expect_match(w, "p.value was not included in var.parametricTerms", all = FALSE)
  w <- capture_warnings(mygam_onlySmoothTermsStat <- ModelArray.gam(FD ~ s(age) + sex,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
  ...
  # # s(age + factorA)   # not good example expect_warning(ModelArray.gam(FD ~
  # s(age + factorA), data = modelarray, phenotypes = phenotypes, scalar =
  # scalar_name, element.subset = element.subset, n_cores = 2, pbar = FALSE))  #
  ...
  # duplicated
  temp <- ModelArray.gam(FD ~ s(age) + sex,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
  ...
  ## different settings in mgcv::gam()'s additional arguments: test if the
  # arguments have been passed into analyseOneElement.gam() handling inputs with
  # NA:
  phenotypes_wNA <- phenotypes
  phenotypes_wNA$age[1] <- NA

  # using default methods in `gam()` to handle NA:
  mygam_agewNA <- ModelArray.gam(FD ~ s(age) + sex,
  ...
  %>% isTRUE())
  # using default methods in `gam()` to handle NA + in a formula with
  # interaction terms: WARNING: should not request effect size for `age` without
  # removing NA observations from it! s(x, by=orderedFactor):
  formula <- FD ~ oSex + s(age, k = 4, fx = TRUE) + s(age, by = oSex, fx = TRUE) + factorB # ordered factor
  mygam_agewNA_sby <- ModelArray.gam(
    formula = formula, data = modelarray, phenotypes = phenotypes_wNA, scalar = scalar_name, element.subset = element.subset,
    n_cores = 2, pbar = FALSE
  )
  ...
  mygam_parametric_pCorrect <- ModelArray.gam(FD ~ s(age) + sex,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
  ...
  ### Test: changed R-squared (delta.adj.rsq, partial.rsq) ##### ### one term of
  # interest: reduced model will be FD ~ 1 ##### also, to test whether the
  # delta.adj.rsq and partial.rsq are calculated correctly also, to test whether
  # without "," in s(), the column name could be correctly "s_age.delta.adj.rsq"
  ...
  # partial.rsq:
  expected <- wrapper_partialRsq_gam_modelarray(
  ...
  ##### the statistics should be consistent, when with or without requesting
  ##### changed.rsq: #####
  mygam_changedrsq_oneSmoothTerm_sex <- ModelArray.gam(FD ~ s(age) + sex,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
  ...
  colnames_intersect <- intersect(
    mygam_changedrsq_oneSmoothTerm_sex %>% colnames(),
    mygam_fullOutputs %>% colnames()
  )
  expect_equal(
    mygam_changedrsq_oneSmoothTerm_sex %>% select(colnames_intersect),
    mygam_fullOutputs %>% select(colnames_intersect)
  )
  # compared to fdr:
  expect_equal(
    mygam_changedrsq_oneSmoothTerm_sex$sex.p.value.fdr,
    mygam_parametric_pCorrect$sex.p.value.fdr
  )
  expect_equal(
    mygam_changedrsq_oneSmoothTerm_sex$s_age.p.value.fdr,
  ...
  mygam_changedRsq_twoSmoothTerm_red1 <- ModelArray.gam(FD ~ factorB + s(factorA),
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
    var.model = c("adj.r.squared"),
    n_cores = 2, pbar = FALSE
  )
  compare_expected_results(mygam_changedRsq_twoSmoothTerm_red1, expected.results[["factorB_s-factorA"]])
  mygam_changedRsq_twoSmoothTerm_red2 <- ModelArray.gam(FD ~ factorB + s(age),
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
    var.model = c("adj.r.squared"),
    n_cores = 2, pbar = FALSE
  )
  ...
  expect_equal(
    mygam_changedRsq_twoSmoothTerm$s_factorA.delta.adj.rsq,
    mygam_changedRsq_twoSmoothTerm$model.adj.r.squared - mygam_changedRsq_twoSmoothTerm_red2$model.adj.r.squared
  )
  expect_equal(
    mygam_changedRsq_twoSmoothTerm$factorB.delta.adj.rsq,
  ...
  ##### the statistics should be consistent, when with or without requesting
  ##### changed.rsq: #####
  mygam_twoSmoothTerm_withoutChangedRsq <- ModelArray.gam(FD ~ factorB + s(age) + s(factorA),
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
    # var.model = c("dev.expl", "adj.r.squared"),
    full.outputs = TRUE,
    correct.p.value.smoothTerms = "fdr", correct.p.value.parametricTerms = "fdr",
  ...
  #### test that s(age, k=4) with "," in the term label --> see if the column
  #### name (s_age.delta.adj.rsq) is as expected #####
  mygam_changedRsq_withComma <- ModelArray.gam(FD ~ s(age, k = 4),
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
    var.model = c("dev.expl", "adj.r.squared"),
    changed.rsq.term.index = c(1),
    n_cores = 2, pbar = FALSE
  ...
  expect_true(c(
    "s_age.delta.adj.rsq",
    "s_age.partial.rsq"
  ) %in% colnames(mygam_changedRsq_withComma) %>% all())

  #### invalid request - see my checker in ModelArray.gam() #####
  expect_error(ModelArray.gam(FD ~ s(age, k = 4),
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
  ...
  expect_error(ModelArray.gam(FD ~ 1,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
  ...
  mygam_changedRsq_te <- ModelArray.gam(FD ~ te(age),
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
    changed.rsq.term.index = c(1),
    n_cores = 2, pbar = FALSE
  )
  ...
  expect_true(c(
    "te_age.delta.adj.rsq",
    "te_age.partial.rsq"
  ) %in% colnames(mygam_changedRsq_te) %>% all())

  # invalid formula: invalid parameters in s such as d = ?
  expect_error(ModelArray.gam(FD ~ s(age, d = 1),
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
    changed.rsq.term.index = c(1),
    n_cores = 2, pbar = FALSE
  ))

  #### MANUALLY CHECK: whether the printed output is as expected (not able to
  #### automatically test via expect_output, probably due to crayon package)
  #### #####
  ...
  # to expect: "s(age,factorA):   k = -1 (default);   fx = FALSE (default);   bs
  # = tp, cr"
  ...
  # to expect: "s(age,factorA):   k = 4;   fx = FALSE (default);   bs = tp, tp
  # (default)"
  ...
  # fx should only has one value; otherwise there will be a warning from
  # mgcv::gam()
  ...
  # formula <- FD ~ s(age*factorA) expect_error(ModelArray.gam(formula =
  # formula, data = modelarray, phenotypes = phenotypes, scalar = scalar_name,
  # element.subset = element.subset, n_cores = 2, pbar = FALSE))
  #
  # formula <- FD ~ s(age*factorA) + s(age)  # will generate duplicated smooth
  # terms of s(age) (as it cannot digest * or +)
  # expect_error(ModelArray.gam(formula = formula, data = modelarray, phenotypes
  # = phenotypes, scalar = scalar_name, element.subset = element.subset, n_cores
  # = 2, pbar = FALSE))
  #
  # formula <- FD ~ s(age + factorA) + s(age)  # will generate duplicated smooth
  # terms of s(age) (as it cannot digest * or +)
  ...
  # s(age * factorA) and s(age + factorA): mgcv::gam throws a warning; column
  # name is NOT correct for .statistics etc; but correct for delta.adj.rsq; also
  # need to test out writing to .h5 may try out interpret.gam(formula) to fix
  # .statistics ModelArray.gam(FD ~ s(age * factorA), data = modelarray,
  # phenotypes = phenotypes, scalar = scalar_name, element.subset =
  # element.subset, changed.rsq.term.index = c(1), n_cores = 2, pbar = FALSE)
  # ModelArray.gam(FD ~ s(age + factorA), data = modelarray, phenotypes =
  # phenotypes, scalar = scalar_name, element.subset = element.subset,
  # changed.rsq.term.index = c(1), n_cores = 2, pbar = FALSE)
  #

  ### check for GAMs with interaction term #####
  ## s(age, by=oSex):
  formula <- FD ~ oSex + s(age, k = 4, fx = TRUE) + s(age, by = oSex, fx = TRUE) + factorB # ordered factor
  mygam_sby <- ModelArray.gam(
    formula = formula,
    data = modelarray,
    phenotypes = phenotypes,
    scalar = scalar_name,
  ...
  expect_true("oSex.L.estimate" %in% colnames(mygam_sby))
  # parametric term | L: linear parameter (vs Q: quadratic; C: cubic)
  expect_true("s_age.statistic" %in% colnames(mygam_sby))
  # (regular) smooth term
  expect_true("s_age_BYoSexM.p.value" %in% colnames(mygam_sby))
  # interaction term | ordered factor, displayed group other than reference group
  expect_true("s_age_BYoSex.delta.adj.rsq" %in% colnames(mygam_sby))
  # interaction term's changed.rsq, as it's the term itself, there is no label for group name (such as "M")
  expect_true("s_age_BYoSex.partial.rsq" %in% colnames(mygam_sby))

  red.formula <- FD ~ oSex + s(age, k = 4, fx = TRUE) + factorB
  mygam_sby.red1 <- ModelArray.gam(
    formula = red.formula,
    data = modelarray,
    phenotypes = phenotypes,
    scalar = scalar_name,
  ...
  # check if delta.adj.rsq is as expected for interaction term (manually
  # calculate the diff of adj.r.sq):
  expect_equal(
    mygam_sby$model.adj.r.squared - mygam_sby.red1$model.adj.r.squared,
    mygam_sby$s_age_BYoSex.delta.adj.rsq
  )

  red.formula <- FD ~ oSex + s(age, by = oSex, fx = TRUE) + factorB
  mygam_sby.red2 <- ModelArray.gam(
    formula = red.formula,
    data = modelarray,
    phenotypes = phenotypes,
    scalar = scalar_name,
  ...
  compare_expected_results(
    mygam_sby.red2,
    expected.results[["oSex_s-age-byoSex-fx-T_factorB"]]
  )
  # check if delta.adj.rsq is as expected for regular smooth term (manually
  # calculate the diff of adj.r.sq): HOWEVER IT MAY DEVIATED FROM ITS TRUE
  # DEFINITION WHEN FORMULA CONTAINS INTERACTION VARIABLES....
  expect_equal(
    mygam_sby$model.adj.r.squared - mygam_sby.red2$model.adj.r.squared,
    mygam_sby$s_age.delta.adj.rsq
  )

  red.formula <- FD ~ s(age, k = 4, fx = TRUE) + s(age, by = oSex, fx = TRUE) + factorB
  mygam_sby.red3 <- ModelArray.gam(
    formula = red.formula,
    data = modelarray,
    phenotypes = phenotypes,
    scalar = scalar_name,
  ...
  compare_expected_results(
    mygam_sby.red3,
    expected.results[["s-age-k-4-fx-T_s-age-byoSex-fx-T_factorB"]]
  )
  # check if delta.adj.rsq is as expected for parametric term (ordered factor)
  # (manually calculate the diff of adj.r.sq): HOWEVER IT MAY DEVIATED FROM ITS
  # TRUE DEFINITION WHEN FORMULA CONTAINS INTERACTION VARIABLES....
  expect_equal(
    mygam_sby$model.adj.r.squared - mygam_sby.red3$model.adj.r.squared,
    mygam_sby$oSex.delta.adj.rsq
  )


  ## interaction terms with multiple levels: make sure s(age, by=oMultiLevels)
  ## output column names are as expected:
  formula <- FD ~ oMultiLevels + s(age, k = 4, fx = TRUE) + s(age, by = oMultiLevels, fx = TRUE) + factorB
  mygam_sby_multiLevels <- ModelArray.gam(
    formula = formula,
    data = modelarray,
    phenotypes = phenotypes,
    scalar = scalar_name,
  ...
  # expected values:
  compare_expected_results(
    mygam_sby_multiLevels,
    expected.results[["oMultiLevels_s-age-k-4-fx-T_s-age-byoMultiLevels-fx-T_factorB"]]
  )
  # column names are as expected:
  expect_true(c(
    "s_age_BYoMultiLevelsB.statistic", # s_age + BY + oMultiLevels + B or C
    "s_age_BYoMultiLevelsC.statistic"
  ) %in% colnames(mygam_sby_multiLevels) %>% all()) # all tested names are in the colnames
  expect_false(
    c("s_age_BYoMultiLevelsA.statistic")
    # s_age + BY + oMultiLevels + A is not in (as A is reference group)
  ...
  # ti(x) + ti(z) + ti(x,z):
  formula <- FD ~ ti(age, fx = TRUE) + ti(factorB, fx = TRUE) + ti(age, factorB, fx = TRUE) + factorA
  mygam_tiInteract <- ModelArray.gam(
    formula = formula,
    data = modelarray,
    phenotypes = phenotypes,
    scalar = scalar_name,
    element.subset = element.subset,
    changed.rsq.term.index = c(3),
    var.model = c("dev.expl", "adj.r.squared"),
    n_cores = 2,
    pbar = FALSE
  )
  compare_expected_results(
    mygam_tiInteract,
    expected.results[["ti-age-fx-T_ti-factorB-fx-T_ti-age-factorB-fx-T_factorA"]]
  )
  ...
  mygam_tiInteract_red1 <- ModelArray.gam(
    formula = red.formula,
    data = modelarray,
    phenotypes = phenotypes,
    scalar = scalar_name,
  ...
  compare_expected_results(
    mygam_tiInteract_red1,
    expected.results[["ti-age-fx-T_ti-factorB-fx-T_factorA"]]
  )
  expect_equal(
    mygam_tiInteract$model.adj.r.squared - mygam_tiInteract_red1$model.adj.r.squared,
  ...
  mygam_tifxF_tiInteract <- ModelArray.gam(
    formula = formula,
    data = modelarray,
    phenotypes = phenotypes,
    scalar = scalar_name,
  ...
  compare_expected_results(
    mygam_tifxF_tiInteract,
    expected.results[["ti-age-fx-T_ti-factorB-fx-F_ti-age-factorB-fx-T_factorA"]]
  )
  expect_true("ti_age.statistic" %in% colnames(mygam_tiInteract))
  expect_true("ti_age_factorB.p.value" %in% colnames(mygam_tiInteract))
  expect_true("ti_age_factorB.delta.adj.rsq" %in% colnames(mygam_tiInteract))
  ...
  mygam_tifxF_tiInteract_red1 <- ModelArray.gam(
    formula = red.formula,
    data = modelarray,
    phenotypes = phenotypes,
    scalar = scalar_name,
  ...
  compare_expected_results(
    mygam_tifxF_tiInteract_red1,
    expected.results[["ti-age-fx-T_ti-factorB-fx-F_factorA"]]
  )
  expect_equal(
    mygam_tifxF_tiInteract$model.adj.r.squared - mygam_tifxF_tiInteract_red1$model.adj.r.squared,
    mygam_tifxF_tiInteract$ti_age_factorB.delta.adj.rsq
  )

  # s(x) + s(z) + ti(x,z):
  formula <- FD ~ s(age, fx = TRUE) + s(factorB, fx = TRUE) + ti(age, factorB, fx = TRUE) + factorA
  mygam_s_tiInteract <- ModelArray.gam(
    formula = formula,
    data = modelarray,
    phenotypes = phenotypes,
    scalar = scalar_name,
  ...
  compare_expected_results(
    mygam_s_tiInteract,
    expected.results[["s-age-fx-T_s-factorB-fx-T_ti-age-factorB-fx-T_factorA"]]
  )
  ...
  mygam_s_tiInteract_red1 <- ModelArray.gam(
    formula = red.formula,
    data = modelarray,
    phenotypes = phenotypes,
    scalar = scalar_name,
  ...
  expect_equal(
    mygam_s_tiInteract$model.adj.r.squared - mygam_s_tiInteract_red1$model.adj.r.squared,
  ...
  # mygam_sby_unordered <- ModelArray.gam(
  #   formula = formula, data = modelarray, phenotypes = phenotypes,
  #   scalar = scalar_name, element.subset = element.subset,
  #   changed.rsq.term.index = c(1,2,3), var.model = c("dev.expl","adj.r.squared"),
  #   n_cores = 2, pbar = FALSE
  # )
  #
  # expect_true("s_age.delta.adj.rsq" %in% colnames(mygam_sby_unordered))
  # check if correct colname - without other specification in s()
  # expect_true("s_age.partial.rsq" %in% colnames(mygam_sby_unordered))
  # expect_true("s_age_BYsexFactorF.statistic" %in% colnames(mygam_sby_unordered))
  # as unordered, there are terms ending with "F" and "M" afer var name "sex_factor"
  ...
  expect_true(
    (length(osex.class) == 2) &
      (osex.class[1] == "ordered") &
      (osex.class[2] == "factor")
  )

  expect_error(
    gen_gamFormula_factorXsmooth(
      response.var = "FD",
      factor.var = "sex",
      smooth.var = "age",
      phenotypes = phenotypes
  ...
  # change k and fx:
  myFormula_3 <- gen_gamFormula_factorXsmooth(
    response.var = "FD",
    factor.var = "oSex",
    smooth.var = "age",
    phenotypes = phenotypes,
