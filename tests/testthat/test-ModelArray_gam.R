test_that("test that ModelArray.gam() works as expected", {
  ...
  mygam_noSmoothTerm <- ModelArray.gam(FD ~ age,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
    n_cores = 2, pbar = FALSE
  ) # should without error or warning, as it's just a message # expect_output() to test output "there is no smooth term in the requested formula" does not work
  ...
  w <- capture_warnings(mygam_onlyModelStat <- ModelArray.gam(FD ~ s(age) + sex,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
    var.smoothTerms = c(), var.parametricTerms = c(), var.model = c("dev.expl"),
    changed.rsq.term.index = c(1),
    n_cores = 2, pbar = FALSE
  )) # warning: p.value was not included in var.smoothTerms OR var.parametricTerms, so not to perform its p.value corrections
  ...
  expect_error(ModelArray.gam(FD ~ s(age) + sex,
    data = modelarray, phenotypes = phenotypes_wrong2, scalar = scalar_name, element.subset = element.subset,
    n_cores = 2, pbar = FALSE
  ))
