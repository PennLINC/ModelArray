test_that("test that ModelArray.gam() works as expected", {
  ...
  ### basic checks #####
  mygam <- ModelArray.gam(FD ~ s(age) + sex, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
  ...
  mygam_default <- ModelArray.gam(FD ~ s(age) + sex, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                                  n_cores = 1, pbar = FALSE)   # default stat outputs
  expect_equal(as.numeric(dim(mygam_default)), c(length(fixel.subset),10))

  mygam_fullOutputs <- ModelArray.gam(FD ~ s(age) + sex, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
  ...
  mygam_noExplicitParamTerm <- ModelArray.gam(FD ~ s(age), data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                                  n_cores = 2, pbar = FALSE)  
  mygam_noParamStat <- ModelArray.gam(FD ~ s(age)+sex, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                                  var.parametricTerms = c(),
                                  n_cores = 2, pbar = FALSE)  
  # if there is no smooth term or stat:
  mygam_noSmoothTerm <- ModelArray.gam(FD ~ age, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
  ...
  mygam_noSmoothStat <- ModelArray.gam(FD ~ s(age) + sex, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                                       var.smoothTerms = c(),
                                       n_cores = 2, pbar = FALSE) 
  expect_false("age.statistic" %in% colnames(mygam_noSmoothStat))
  
  # if there is no model stat:
  mygam_noModelStat <- ModelArray.gam(FD ~ s(age) + sex, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                                      var.model = c(),
                                      n_cores = 2, pbar = FALSE) 
  
  # no any model stat:
  expect_error(ModelArray.gam(FD ~ s(age) + sex, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
  ...
  mygam_sAge_sFactorA <- ModelArray.gam(FD ~ s(age) + s(factorA), data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                                n_cores = 2, pbar = FALSE)
  
  # # s(age + factorA)   # not good example
  # expect_warning(ModelArray.gam(FD ~ s(age + factorA), data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
  ...
  expect_error(ModelArray.gam(FD ~ s(age) + sex, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                              ...
                              n_cores = 2, pbar = FALSE))
  # duplicated
  temp <- ModelArray.gam(FD ~ s(age) + sex, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
  ...
  # s(k=?):
  mygam_k4 <- ModelArray.gam(FD ~ s(age, k=4) + sex, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                             n_cores = 2, pbar = FALSE)
  ...
  mygam_fxTRUE <- ModelArray.gam(FD ~ s(age, fx=TRUE) + sex, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                                 n_cores = 2, pbar = FALSE)
  expect_false(all_equal(mygam_default %>% dplyr::select("s_age.statistic"),
  ...
  # s(bs=?)
  mygam_bsCR <- ModelArray.gam(FD ~ s(age, bs="cr") + sex, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                               n_cores = 2, pbar = FALSE)
  expect_false(all_equal(mygam_default %>% dplyr::select("s_age.statistic"),
  ...
  # method: 
  mygam_methodREML <- ModelArray.gam(FD ~ s(age) + sex, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
  ...
  # n_cores = 2: 
  mygam_pbarFalse_ncores2 <- ModelArray.gam(FD ~ s(age) + sex, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                                           n_cores = 2, pbar = FALSE)   # default stat outputs
  expect_equal(mygam_default, mygam_pbarFalse_ncores2)
  
  # pbar:
  mygam_pbarTrue_ncores2 <- ModelArray.gam(FD ~ s(age) + sex, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                                  n_cores = 2, pbar = TRUE)   # default stat outputs
  expect_equal(mygam_default, mygam_pbarTrue_ncores2)
  
  mygam_pbarTrue_ncores1 <- ModelArray.gam(FD ~ s(age) + sex, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                                           n_cores = 1, pbar = TRUE)   # default stat outputs
  expect_equal(mygam_default, mygam_pbarTrue_ncores1)
  
  
  ### Test: p.value correction: #####
  mygam_parametric_pCorrect <- ModelArray.gam(FD ~ s(age) + sex, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
  ...
  mygam_smooth_pCorrect <- ModelArray.gam(FD ~ s(age) + sex, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
  ...
  expect_error(ModelArray.gam(FD ~ s(age) + sex, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                              ...
                              n_cores = 2, pbar = FALSE))
  expect_warning(ModelArray.gam(FD ~ s(age) + sex, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
  ...
  mygam_effsize_oneSmoothTerm <- ModelArray.gam(FD ~ s(age), data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                                               var.model = c("dev.expl", "adj.r.squared"),
                                               eff.size.term.index = c(1),
                                               n_cores = 2, pbar = FALSE)
  mygam_intercept <- ModelArray.gam(FD ~ 1, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
  ...
  mygam_effsize_twoSmoothTerm <- ModelArray.gam(FD ~ factorB + s(age) + s(factorA), data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                                               var.model = c("dev.expl", "adj.r.squared"),
                                               eff.size.term.index = c(1,2,3),
                                               n_cores = 2, pbar = FALSE)
  mygam_effsize_twoSmoothTerm_red1 <- ModelArray.gam(FD ~ factorB + s(factorA), data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                                                     var.model = c( "adj.r.squared"),
                                                     n_cores = 2, pbar = FALSE)
  mygam_effsize_twoSmoothTerm_red2 <- ModelArray.gam(FD ~ factorB + s(age), data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                                                     var.model = c( "adj.r.squared"),
                                                     n_cores = 2, pbar = FALSE)
  mygam_effsize_twoSmoothTerm_red3 <- ModelArray.gam(FD ~ s(age) + s(factorA), data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                                                     var.model = c( "adj.r.squared"),
                                                     n_cores = 2, pbar = FALSE)
  ...
  mygam_effsize_withComma <- ModelArray.gam(FD ~ s(age, k=4), data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                 var.model = c("dev.expl", "adj.r.squared"),
                 eff.size.term.index = c(1),
                 n_cores = 2, pbar = FALSE)
  ...
  expect_error(ModelArray.gam(FD ~ s(age, k=4), data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                              eff.size.term.index = c(0),   # index is < 1
                              n_cores = 2, pbar = FALSE))
  expect_error(ModelArray.gam(FD ~ s(age, k=4), data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                              eff.size.term.index = c(2),   # index is more than # of terms
                              n_cores = 2, pbar = FALSE))
  expect_error(ModelArray.gam(FD ~ 1, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
  ...
  mygam_effsize_te <- ModelArray.gam(FD ~ te(age), data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                                     eff.size.term.index = c(1),  
                                     n_cores = 2, pbar = FALSE)
  expect_true("te_age.eff.size" %in% colnames(mygam_effsize_te))
  
  # invalid formula: invalid parameters in s such as d = ?
  expect_error(ModelArray.gam(FD ~ s(age, d=1), data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                 eff.size.term.index = c(1),
                 n_cores = 2, pbar = FALSE))
  ...
  ModelArray.gam(formula = formula, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                 eff.size.term.index = c(1),
                 n_cores = 2, pbar = FALSE)
  ...
  # expect_error(ModelArray.gam(formula = formula, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
  #                             n_cores = 2, pbar = FALSE))
  # 
  ...
  # ModelArray.gam(FD ~ s(age * factorA), data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
  #                eff.size.term.index = c(1),
  #                n_cores = 2, pbar = FALSE)
  # ModelArray.gam(FD ~ s(age + factorA), data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
  #                eff.size.term.index = c(1),
  ...
  mygam_sby <- ModelArray.gam(formula = formula, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
  ...
  mygam_sby.red1 <- ModelArray.gam(formula = red.formula, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                                  var.model = c("dev.expl","adj.r.squared"),
                                  n_cores = 2, pbar = FALSE)
  ...
  mygam_sby.red2 <- ModelArray.gam(formula = red.formula, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                                  var.model = c("dev.expl","adj.r.squared"),
                                  n_cores = 2, pbar = FALSE)
  ...
  mygam_sby.red3 <- ModelArray.gam(formula = red.formula, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                                   var.model = c("dev.expl","adj.r.squared"),
                                   n_cores = 2, pbar = FALSE)
  ...
  mygam_tiInteract <- ModelArray.gam(formula = formula, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
  ...
  mygam_tiInteract_red1 <- ModelArray.gam(formula = red.formula, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                                          var.model = c("adj.r.squared"),
                                          n_cores = 2, pbar = FALSE)
  ...
  mygam_sby_unordered <- ModelArray.gam(formula = formula, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, fixel.subset = fixel.subset,
                              eff.size.term.index = c(1,2,3), var.model = c("dev.expl","adj.r.squared"),
                              n_cores = 2, pbar = FALSE)
