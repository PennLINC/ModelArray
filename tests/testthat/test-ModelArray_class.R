test_that("ModelArray interface works as expected", {
  ...
  tryCatch(
    {
      ...
      element.subset <- 1:10
      # fit lm:
      mylm <- ModelArray.lm(FD ~ age, data = modelarray_output, phenotypes = phenotypes, scalar = "FD", element.subset = element.subset, 
                            n_cores = 2, pbar=FALSE)
      mylm_fulloutput <- ModelArray.lm(FD ~ age, data = modelarray_output, phenotypes = phenotypes, scalar = "FD", element.subset = element.subset, 
                                       full.outputs = TRUE,
                                       n_cores = 2, pbar=FALSE)
      ...
      # re-load:
      modelarray_new <- ModelArray(h5_path_output, scalar_types = c("FD"),
  ...
  # numElementsTotal():
  expect_equal(numElementsTotal(modelarray, "FD"), 
               num.fixels)
