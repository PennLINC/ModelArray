...
test_that("ModelArray handles subject-specific masks as expected", {
  ...
  check_as_expected <- function(df.results, expectedResults,
    ...
    if (is.null(element.subset)) {
      expect_equal(df.results$element_id,
                   0:(nrow(df.results)-1) )
    } else {
      expect_equal(df.results$element_id,
                   element.subset)  # otherwise, use the one specified
    }
  ...
  for (i.config in 1:nrow(df.config)) {
    ...
    if (specials == "") {   # specials=NULL
      ...
      mylm.fullOutputs <- ModelArray.lm(FA ~ age + sex, modelarray, phenotypes, scalar = scalar_name, element.subset = NULL,
                                        full.outputs = TRUE, 
                                        correct.p.value.terms = c("fdr", "bonferroni"),correct.p.value.model = c("fdr", "bonferroni"),
                                        n_cores = 2)
      expect_equal(check_as_expected(mylm.fullOutputs, expectedResults),
                   rep(TRUE, nrow(mylm.fullOutputs)))
      
      
      ### ModelArray.gam()
      mygam <- ModelArray.gam(FA ~ s(age) + sex, modelarray, phenotypes, scalar = scalar_name, element.subset = NULL,
                              n_cores = 2)
      expect_equal(check_as_expected(mygam, expectedResults),
                   rep(TRUE, nrow(mygam)))
      
      mygam.fullOutputs <- ModelArray.gam(FA ~ s(age) + sex, modelarray, phenotypes, scalar = scalar_name, element.subset = NULL,
                                          full.outputs = TRUE,
                                          correct.p.value.smoothTerms = c("fdr", "bonferroni"), correct.p.value.parametricTerms = c("fdr", "bonferroni"),
                                          n_cores = 2)
      expect_equal(check_as_expected(mygam.fullOutputs, expectedResults),
                   rep(TRUE, nrow(mygam.fullOutputs)))
      
      mygam.changed.rsq <- ModelArray.gam(FA ~ s(age) + sex + factorA, modelarray, phenotypes, scalar = scalar_name, element.subset = NULL,
                                          changed.rsq.term.index = c(1),
                                          n_cores = 2)
      expect_equal(check_as_expected(mygam.changed.rsq, expectedResults),
                   rep(TRUE, nrow(mygam.changed.rsq)))
      
      ### if threshold changes, expected results will also change, esp those around thr   #####
      # num.subj.lthr.abs
      mylm.changed.abs.thr <- ModelArray.lm(FA ~ age, modelarray, phenotypes, scalar = scalar_name, element.subset = NULL,
                                        num.subj.lthr.abs = 7, 
                                        n_cores = 2)
      mygam.changed.abs.thr <- ModelArray.gam(FA ~ s(age), modelarray, phenotypes, scalar = scalar_name, element.subset = NULL,
                                             num.subj.lthr.abs = 7, 
                                             n_cores = 2)
      # num.subj.lthr.rel
      mylm.changed.rel.thr <- ModelArray.lm(FA ~ age, modelarray, phenotypes, scalar = scalar_name, element.subset = NULL,
                                            num.subj.lthr.rel = 0.3, 
                                            n_cores = 2)
      mygam.changed.rel.thr <- ModelArray.gam(FA ~ s(age), modelarray, phenotypes, scalar = scalar_name, element.subset = NULL,
                                              num.subj.lthr.rel = 0.3, 
                                              n_cores = 2)
      
      if (nsubj == 60) {
        # with new abs.thr: does not affect it:
        expect_equal(check_as_expected(mylm.changed.abs.thr, expectedResults),    
                     rep(TRUE, nrow(mylm.changed.abs.thr)))   # still as expected, as 60*0.2 = 12 creating a higher bar
        expect_equal(check_as_expected(mygam.changed.abs.thr, expectedResults),    
                     rep(TRUE, nrow(mygam.changed.abs.thr)))   # still as expected, as 60*0.2 = 12 creating a higher bar
        ...
        # now:      0,0,0,0,0,0
        updated_temp <- c(rep(TRUE, 10),   # TRUE: consistent with expectedResults saved in .txt; FALSE; inconsistent
                          c(FALSE, TRUE, TRUE, TRUE, TRUE, TRUE),   
                          rep(TRUE, 4),
                          rep(TRUE, nrow(mylm.changed.rel.thr)-20))
        expect_equal(check_as_expected(mylm.changed.rel.thr, expectedResults),    
                     updated_temp)
        expect_equal(check_as_expected(mygam.changed.rel.thr, expectedResults),    
                     updated_temp)
