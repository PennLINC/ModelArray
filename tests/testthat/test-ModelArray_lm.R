test_that("ModelArray.lm() works as expected", {
  ...
  expect_equal(as.numeric(dim(mylm)), c(
    length(element.subset), # check shape
    1 +
  ...
  mylm_age_sex <- ModelArray.lm(FD ~ age + sex,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
    var.terms = var.terms,
    var.model = var.model,
    n_cores = 2, pbar = FALSE
  ...
  expect_equal(as.numeric(dim(mylm_age_sex)), c(
    length(element.subset),
    1 +
  ...
  expect_error(ModelArray.lm(FD ~ age,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
  ...
  expect_warning(mylm_noTermsOutput <- ModelArray.lm(FD ~ age,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
    var.terms = c(), var.model = var.model,
    n_cores = 1, pbar = FALSE
  )) # expect warning because there will be a warning from ModelArray.lm(): p.value was not included in var.terms, so not to perform its p.value corrections
  compare_expected_results(mylm_noTermsOutput, expected.results$age)
  expect_equal(as.numeric(dim(mylm_noTermsOutput)), c(
    length(element.subset),
    1 +
      length(var.model) + 1
  )) # check shape
  ...
  expect_warning(mylm_noModelOutput <- ModelArray.lm(FD ~ age,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
    var.terms = var.terms, var.model = c(),
    n_cores = 1, pbar = FALSE
  )) # expect warning because there will be a warning from ModelArray.lm(): p.value was not included in var.model, so not to perform its p.value corrections
  compare_expected_results(mylm_noModelOutput, expected.results$age)
  expect_equal(as.numeric(dim(mylm_noModelOutput)), c(
    length(element.subset),
    1 +
  ...
  expect_error(mylm_noModelOutput <- ModelArray.lm(FD ~ age,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
  ...
  mylm_corr_pvalues_3 <- ModelArray.lm(FD ~ age,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
    var.terms = var.terms, var.model = var.model,
  ...
  expect_true(c(
    "age.p.value.bonferroni", "Intercept.p.value.bonferroni",
    "model.p.value.fdr"
  ) %in% colnames(mylm_corr_pvalues_3) %>% all()) # correct.p.value.terms: bonferroni is in, but not fdr
  ...
  mylm_corr_pvalues_4 <- ModelArray.lm(FD ~ age,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
    var.terms = var.terms, var.model = var.model,
  ...
  ) %in% colnames(mylm_corr_pvalues_4) %>% all()) # correct.p.value.model: bonferroni is in, but not fdr
  ...
  expect_warning(temp <- ModelArray.lm(FD ~ age,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
  ...
  mylm_age_factorA <- ModelArray.lm(FD ~ age + factorA,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
  ...
  mylm_age_factorB <- ModelArray.lm(FD ~ age + factorB,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
    var.terms = var.terms.full,
    var.model = var.model,
    n_cores = 2, pbar = FALSE
  ...
  mylm_phenotypes_naActionDefault <- ModelArray.lm(FD ~ age,
    data = modelarray, phenotypes = phenotypes_wNA,
  ...
  expect_error(ModelArray.lm(FD ~ age,
    ...
    na.action = "na.fail"
  )) # expect error of "missing values in object". If na.action was not passed into lm, there will not be error
  ...
  expect_error(ModelArray.lm(FD ~ age,
    ...
    na.action = "na.fail"
  )) # NOTE: after updating ModelArray.lm with one row, specifying column names to keep, expect error (instead of warning for each core with error, expect_warning)
  ...
  expect_error(ModelArray.lm(FD ~ age,
    ...
    na.action = "na.fail"
  )) # after updating ModelArray.lm with one row, specifying column names to keep, expect error (instead of warning for each core with error, expect_warning)

  # okay if `na.action = "na.omit"`:
  mylm_phenotypes_naActionOmit <- ModelArray.lm(FD ~ age,
    data = modelarray, phenotypes = phenotypes_wNA, scalar = scalar_name, element.subset = element.subset,
    var.terms = var.terms, var.model = var.model, n_cores = 2, pbar = FALSE,
    na.action = "na.omit"
  )
  ...
  mylm_weights1 <- ModelArray.lm(FD ~ age,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
    var.terms = var.terms, var.model = var.model,
    pbar = FALSE, n_cores = 2,
    weights = rep(1, nrow(phenotypes))
  ) # length(phenotypes$subject_id)   # weights = rep(1,nrow(phenotypes))
  expect_equal(mylm, mylm_weights1)


  set.seed(num.set.seed)
  mylm_weightsRnorm <- ModelArray.lm(FD ~ age,
    data = modelarray, phenotypes = phenotypes, scalar = scalar_name, element.subset = element.subset,
    var.terms = var.terms,
    var.model = var.model,
    n_cores = 2, pbar = FALSE,
  ...
  mylm_testSwap <- ModelArray.lm(FD ~ age,
