test_that("ModelArray.lm() works as expected", {
  ...
  expect_equal(mylm$grid_id, 0:99)   # check output$grid_id 
  ...
  mylm_default <- ModelArray.lm(FD ~ age, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, grid.subset = 1:100, 
                                n_cores = 2, pbar=FALSE)   # default full.outputs and var.*
  expect_equal(as.numeric(dim(mylm_default)), c(100,1+2*3+2)) # check shape  
  
  mylm_fullOutputs <- ModelArray.lm(FD ~ age, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, grid.subset = 1:100, 
                                    full.outputs = TRUE,   # default: FALSE
                                n_cores = 2, pbar=FALSE)   
  expect_equal(as.numeric(dim(mylm_fullOutputs)), c(100,21))
  
  
  mylm_age_sex <- ModelArray.lm(FD ~ age + sex, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, grid.subset = 1:100, 
                                var.terms = var.terms,
                                var.model = var.model,
                                n_cores = 2, pbar=FALSE)
  expect_equal(mylm_age_sex$grid_id, 0:99)   # check output$grid_id 
  ...
  expect_error(ModelArray.lm(FD ~ age, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, grid.subset = 1:100, 
                             ...
                             var.model = var.model, 
                             n_cores = 2, pbar=FALSE))
  temp <- (ModelArray.lm(FD ~ age, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, grid.subset = 1:100, 
                             var.terms = var.terms,
  ...
  # n_cores=2:
  mylm_ncores2 <- ModelArray.lm(FD ~ age, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, grid.subset = 1:100, 
                                var.terms = var.terms,
                                var.model = var.model, 
                                n_cores = 2, pbar=FALSE)
  expect_equal(mylm, mylm_ncores2)
  # pbar=TRUE & n_cores=1
  mylm_pbarTRUE_ncores1 <- ModelArray.lm(FD ~ age, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, grid.subset = 1:100, 
                                         var.terms = var.terms,
                                         var.model = var.model,
                                         n_cores = 1, pbar=TRUE)
  expect_equal(mylm, mylm_pbarTRUE_ncores1)
  # pbar=TRUE & n_cores=2
  mylm_pbarTRUE_ncores2 <- ModelArray.lm(FD ~ age, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, grid.subset = 1:100, 
                                         var.terms = var.terms,
                                         var.model = var.model,
                                         n_cores = 2, pbar=TRUE)
  ...
  mylm_noTermsOutput <- ModelArray.lm(FD ~ age, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, grid.subset = 1:100, 
                              var.terms = c(),
                              var.model = var.model,
                              n_cores = 1, pbar=FALSE)
  expect_equal(as.numeric(dim(mylm_noTermsOutput)), c(100,1+length(var.model))) # check shape
  
  mylm_noModelOutput <- ModelArray.lm(FD ~ age, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, grid.subset = 1:100, 
                                      var.terms = var.terms,
                                      var.model = c(),
                                      n_cores = 1, pbar=FALSE)
  ...
  # terms:
  mylm_corr_pvalues_1 <- ModelArray.lm(FD ~ age, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, grid.subset = 1:100, 
  ...
  expect_warning( ModelArray.lm(FD ~ age, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, grid.subset = 1:100, 
  ...
  mylm_age_factorA <- ModelArray.lm(FD ~ age + factorA, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, grid.subset = 1:100, 
                                    var.terms = var.terms.full,
                                    var.model = var.model, 
                                    n_cores = 2, pbar=FALSE)
  # temp <- mylm_age_factorA %>% dplyr::filter(term=="factorA")%>% dplyr::select(-term)  # only extracting column "factorA"
  expect_equal(mylm_age_factorA$grid_id, c(0:99))    # test that $grid_id is 0:99
  ...
  mylm_age_factorB <- ModelArray.lm(FD ~ age + factorB, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, grid.subset = 1:100, 
                                    var.terms = var.terms.full,
                                    var.model = var.model, 
                                    n_cores = 2, pbar=FALSE)
  # temp <- mylm_age_factorB %>% dplyr::filter(term=="factorB")%>% dplyr::select(-term) %>% dplyr::select(-grid_id)  
  ...
  expect_error(ModelArray.lm(FD ~ age, data = modelarray, phenotypes = phenotypes_wNA, scalar = scalar_name, grid.subset = 1:100, 
  ...
  #mylm_phenotypes_naActionDefault <- ModelArray.lm(FD ~ age, data = modelarray, phenotypes = phenotypes_wNA, scalar = scalar_name, grid.subset = 1:100, n_cores = 2, pbar=FALSE)
  mylm_phenotypes_naActionOmit <- ModelArray.lm(FD ~ age, data = modelarray, phenotypes = phenotypes_wNA, scalar = scalar_name, grid.subset = 1:100, 
                                                var.terms = var.terms, var.model = var.model, n_cores = 2, pbar=FALSE, 
                                                na.action="na.omit")
  ...
  mylm_weights1 <- ModelArray.lm(FD ~ age, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, grid.subset = 1:100, 
                        var.terms = var.terms, var.model = var.model, 
  ...
  set.seed(5)
  mylm_weightsRnorm <- ModelArray.lm(FD ~ age, data = modelarray, phenotypes = phenotypes, scalar = scalar_name, grid.subset = 1:100, 
                                     var.terms = var.terms,
                                     var.model = var.model,
                                     n_cores = 2, pbar=FALSE, 
