...
ModelArray supports within-process parallelism via the `n_cores`
parameter - multiple CPU cores in a single R session working through
elements in parallel. For many datasets, this is sufficient.

However, for very large datasets (e.g., 600,000+ fixels with thousands
of subjects), even a multi-core single job may take too long. In these
cases, you can **split the elements across multiple independent HPC
jobs**, each processing a different range of elements.

## Generating split ranges

Use
[`numElementsTotal()`](https://pennlinc.github.io/ModelArray/reference/numElementsTotal.md)
to get the total element count, then divide it into ranges:
...
Each job runs the same model on its assigned range of elements. For
example, an R script parameterized by job index:
...
After all jobs complete, read each chunk `.h5`, concatenate rows, then
write one final result to `data.h5`:
...
**Important**: only one process should write to the HDF5 file at a time.
HDF5 does not support concurrent writes from multiple processes — see
